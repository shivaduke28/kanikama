...
namespace Kanikama.Editor
{
    ...
    public sealed class BakeTargetHandleV2<T> : IBakeTargetHandle where T : Object, ILightSourceV2
    {
        readonly SceneObjectId sceneObjectId;
        readonly string name;
        ObjectHandle<T> handle;
        ...
        public BakeTargetHandleV2(T value)
        {
            var globalObjectId = GlobalObjectId.GetGlobalObjectIdSlow(value);
            sceneObjectId = new SceneObjectId(globalObjectId);
            name = value.name;
    ...
    public sealed class BakeTargetGroupElementHandleV2<T> : IBakeTargetHandle where T : Object, ILightSourceGroupV2
    {
        readonly SceneObjectId sceneObjectId;
        readonly string name;
        readonly int index;
        ObjectHandle<T> handle;
        ILightSourceV2 GetBakeTarget() => handle.Value.Get(index);

        public BakeTargetGroupElementHandleV2(T value, int index)
        {
            var globalObjectId = GlobalObjectId.GetGlobalObjectIdSlow(value);
            sceneObjectId = new SceneObjectId(globalObjectId);
            this.index = index;
